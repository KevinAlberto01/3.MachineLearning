...
It focuses  Mainly focused on cleaning the data by removing nulls and preparing a base dataset for analysis and modeling.
Load the house prices dataset (AmesHousing.csv), identify numerical and categorical columns, fill in missing values, and finally save a new clean file named AmesHousing_cleaned.csv.

* **1.Load the DataSet** 
    - Read the original file (AmesHousing.csv) and load it into a DataFrame df.

* **2.Column Identification**
    - Identify numeric columns (float64 and int64 types).
    - Identify categorical columns (object type).
    - Example of what it could detect:
        - Numerical: LotArea, YearBuilt, SalePrice, etc
        - Categorical: MSZoning, Neighborhood, GarageType, etc.
...
* **4.Save Clean Dataset**
    - This file does not yet have dummies, that is, it keeps the categorical columns with text (so that the next program can decide what to do with them).

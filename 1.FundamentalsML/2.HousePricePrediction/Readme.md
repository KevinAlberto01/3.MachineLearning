...
## Description
This project involves developing a Machine Learning model capable of estimating the price of a house based on various characteristics, such as location, number of rooms, land size, age, among other factors. It is important as it allows determining the approximate value of a property without the need for a human appraiser.

<p align = "center" >
    <h2 align = "Center">🎯 Objetives 🎯</h2>
</p>

Predict the selling price of a house as accurately as possible, using historical data and relevant features.

Explore and clean the dataset (EDA), select or engineer new variables (feature engineering), and evaluate different regression models (Linear, Decision Trees, Random Forest, XGBoost, etc.).

Optimize the chosen model using techniques like GridSearchCV or RandomSearch (hyperparameters) and evaluate with error metrics (RMSE, MAE, R^2, etc.).

Demonstrate the model's generalization capability using a test set and/or cross-validation.
...
Python (NumPy, Pandas, Matplotlib/Seaborn for EDA, Scikit-Learn for models).
Streamlit or Dash (optional) if you want to create a dashboard to present results.
...
Note: in the accuracy its different because the number to two-place decimal but it the same result that we use the **SVM Algorithm with 98% of Acurrancy** because have better precition and its excelent to small datasets# 1. Handwritten Digits Classifier (MNIST)
...
Note: in the accur# 1. Handwritten Digits Classifier (MNIST)
...
<img src = "https://github.com/KevinAlberto01/3.MachineLearning/blob/main/1.FundamentalsML/1.HandwrittenDigitClassifier(MNIST)/1.8Personalisation/Images/Dashboard.png" width="2000"/>acy its different because the number to two-place decimal but it the same result that we use the **SVM Algorithm with 98% of Acurrancy** because have better precition and its excelent to small datasets
